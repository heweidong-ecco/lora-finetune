...
**要支撑核心论点，还缺一组 E04：固定全部参数，只换数据集**（例如 194 条 vs 500 条，
或 清洗前 vs 清洗后）。这是本仓库当前**最大的缺口**。
...
|---|---|---|---|---|---|---|
| 1 | `55ab5df2b466` | 2e-4 | 3 | **0.0000** | **14.32 s** | ❓ 疑似冒烟测试（loss 恰为 0 + 仅 14 秒） |
...
| 6 | `c0beed18beb7` | 2e-4 | 3 | 1.0588 | 120.52 s | ❓ 无对应实验 |
...
> 📌 **`results/` 里的评测是另一条线**：`evaluation_results.json` / `final_judgment.json`
> 来自 BadCase 管线，比较的是 **基座模型 vs 微调模型**（500 条训练 / 50 条测试），
> **不是** E01/E02/E03 之间的对比。**不要混用。**
